...
class TestHelperFunctions:
    ...
    def test_all_documents_render_footer_at_bottom(self):
        ...
        # 3. file_explanations.md
        assert "**Generated by Project Gnosis (Code Archaeology Agent)**" in result.file_explanations_doc
        assert result.file_explanations_doc.strip().endswith(state.analysis_mode)

    def test_skip_llm_user_friendly_message(self):
        state = make_full_state(with_explanations=False)
        result = run(state)

        # file_explanations_doc should contain the friendly Option D message and footer
        assert "Skipped AI Explanations" in result.file_explanations_doc
        assert "Want file walkthroughs? Just turn on AI explanations and run the analysis again." in result.file_explanations_doc
        assert "**Generated by Project Gnosis (Code Archaeology Agent)**" in result.file_explanations_doc

        # agent_context_md should also contain the friendly Option D message
        assert "Skipped AI Explanations" in result.agent_context_md
        assert "Want file walkthroughs? Just turn on AI explanations and run the analysis again." in result.agent_context_md